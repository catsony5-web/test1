...
4. 설정/관리 → 데이터에서 `전체 데이터 백업`을 내려받아 저장소 밖의 개인 보관 위치에 보관합니다.

전체 백업에는 수기 거래, 수입 거래, 월별 직접 입력 수입, 수정한 금액·분류·메모·정산금이 포함됩니다. 항목 체크박스는 복원과 `선택 항목만 백업`에만 적용됩니다. 기본 병합 복원은 같은 거래나 월 수입의 내용이 다르면 현재 값과 백업 값을 비교해 항목별로 선택하며, 선택 전 취소하면 현재 가계부를 유지합니다. 거래를 선택할 때 해당 정산금도 함께 적용됩니다. 기록 초기화는 별도 선택 영역을 사용하며 처음에는 엑셀 업로드 거래만 선택됩니다.
